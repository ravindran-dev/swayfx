#!/usr/bin/env python3
"""Entry point: toggles the Bluetooth popup (2nd click/invocation closes it).
...
LOCK_PATH = "/run/user/{}/bluetooth-popup.lock".format(os.getuid())
PID_PATH = "/run/user/{}/bluetooth-popup.pid".format(os.getuid())
...
def _close_other_waybar_popups():
    # Mirrors wifi_popup.py's own _close_other_waybar_popups() - without
    # this, opening one popup while another is open leaves both stacked
    # in the same corner (confirmed live during the Wi-Fi popup's build).
    try:
        ...
        pass
    for pid_path in (WIFI_PID_PATH, VOLUME_PID_PATH):
        try:
            with open(pid_path) as f:
...
def main():
    ...
    def quit_app():
        ...
        # native D-Bus worker thread the loop/interpreter shutdown ends up
    ...
    popup = ui.BluetoothPopup(bt, on_closed=quit_app)
